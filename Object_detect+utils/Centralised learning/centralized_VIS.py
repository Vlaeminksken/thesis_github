# coco_train_binary.py  –  3-class version of your original script
# ────────────────────────────────────────────────────────────────────────────
import os
import tensorflow as tf
from tensorflow.keras import layers, models, callbacks
import config4 as config1                      # ← same config file as before

# ─── DEFINE SUPER-CLASSES ───────────────────────────────────────────────────
# • Put here the 0-based IDs of every class that can MOVE on its own or be driven
# ─── DEFINE SUPER-CLASSES (use 0-based IDs!) ────────────────────────────────
# Moving / self-propelled or drivable things
MOVING_CLASSES = {
    0,   # 1  Dog ahead          → 0 after 1-based → 0-based shift
    3,   # 4  auto ahead         → 3
    6,   # 7  bicycle ahead      → 6
    7,   # 8  bus ahead          → 7
    8,   # 9  car ahead          → 8
    9,   # 10 cattle ahead       → 9
    16,  # 17 motorcycle ahead   → 16
    18,  # 19 person ahead       → 18
    27,  # 28 truck ahead        → 27
}

# Everything else is considered stand-still infrastructure / background
STANDSTILL_CLASSES = set(range(30)) - MOVING_CLASSES
...
# Let the rest of the code know we now have three classes
...
# ─── MODEL DEFINITION (only Dense layer size changed) ───────────────────────
def create_model():
...
# ─── TRAIN / VALID / TEST (unchanged apart from save name) ──────────────────
if __name__ == '__main__':
    train_ds = load_dataset(config1.TRAIN_RECORD_PATH,  shuffle=False, batch_size=8, augment_data=True)
    valid_ds = load_dataset(config1.VALID_RECORD_PATH,  shuffle=False, batch_size=8, augment_data=False)
    test_ds  = load_dataset(config1.TEST_RECORD_PATH,   shuffle=False, batch_size=8, augment_data=False)
    ...
    history = model.fit(
        train_ds, validation_data=valid_ds, epochs=20, callbacks=[lr_cb], verbose=1
    )

    loss, acc = model.evaluate(test_ds, verbose=1)
    print(f"\nTest Accuracy: {acc * 100:.2f}%")

    save_path = os.path.join(config1.BASE_DIR, 'SA_classification_binary_VIS11.keras')
